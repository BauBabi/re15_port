...
long g_bits_at_limit = -1; long g_final_bits = -1; int g_blocks = 0; int g_block_limit = 0;
/* ====
 * Public entry point */
                        ...
                        int16_t *dst, size_t dst_capacity)
{
    ...
    g_blocks = 0; g_bits_at_limit = -1;
    while (s.dst_offset < s.dst_capacity) {
        if (g_block_limit > 0 && g_blocks >= g_block_limit) {
            g_bits_at_limit = (long)(s.src_offset * 8) + s.bit_count - 16;
            break;
        }
    ...
    if (g_bits_at_limit < 0)
        g_bits_at_limit = (long)(s.src_offset * 8) + s.bit_count - 16;
    g_final_bits = (long)(s.src_offset * 8) + s.bit_count - 16;
