...
quads=[]
for i in range(15):
    f=struct.unpack_from("<8H",d,base+0x170+i*16); quads.append((f[1],f[3],f[5],f[7]))
PROP=(-7124,0,-6976)
# SCA first group (19 entries)
S=0x570; sca=[]
for i in range(19):
    o=S+24+i*12
    w,dn,x,z,ty,u0,u1,fl = struct.unpack_from("<HHhhBBBB", d, o)
    sca.append((x,z,w,dn,ty))

def build_view(px,py,pz,tx,ty,tz,fov):
    dx,dy,dz = tx-px, ty-py, tz-pz
    dist = math.isqrt(dx*dx+dy*dy+dz*dz); horiz= math.isqrt(dx*dx+dz*dz)
    def s16c(v):
        v&=0xffff; return v-0x10000 if v>=0x8000 else v
    sp = s16c(int((-dy*4096)//dist)) if dist else 0
    cp = s16c(int((horiz*4096)//dist)) if dist else 4096
    if horiz:
    ...
    t=[(R[0]*-px+R[1]*-py+R[2]*-pz)>>12,(R[3]*-px+R[4]*-py+R[5]*-pz)>>12,(R[6]*-px+R[7]*-py+R[8]*-pz)>>12]
    return R,t,(fov>>7)
def proj(R,t,H,w):
    cx=((R[0]*w[0]+R[1]*w[1]+R[2]*w[2])>>12)+t[0]
    cy=((R[3]*w[0]+R[4]*w[1]+R[5]*w[2])>>12)+t[1]
    cz=((R[6]*w[0]+R[7]*w[1]+R[8]*w[2])>>12)+t[2]
    if cz<=1: return None
    return (160+cx*H//cz, 120+cy*H//cz, cz)
def readpng(p):
...
for ci in range(10):
    flag,fov,cpx,cpy,cpz,tx,ty,tz,pri = struct.unpack_from("<HHiiiiiiI", d, 0x60+ci*32)
    R,t,H = build_view(cpx,cpy,cpz,tx,ty,tz,fov)
    w,h,img = readpng(os.path.join(OUT,"bg%02d.png"%ci))
    # SCA blockers as floor rectangles (cyan) + top edge at y=-4000 (blue)
    for (x,z,ww,dd,ty_) in sca:
        if ty_!=1: continue
        c=[(x,z),(x+ww,z),(x+ww,z+dd),(x,z+dd)]
        for yy,col in ((0,(0,255,255)),(-4000,(0,120,255))):
            ps=[proj(R,t,H,(cx,yy,cz)) for cx,cz in c]
            for a in range(4):
                pa,pb=ps[a],ps[(a+1)%4]
                if pa and pb: line(img,w,h,pa[0],pa[1],pb[0],pb[1],col)
    # prop in red
    pts=[proj(R,t,H,(v[0]+PROP[0],v[1]+PROP[1],v[2]+PROP[2])) for v in verts]
    for q in quads:
        for a,b in ((0,1),(1,2),(2,3),(3,0)):
            pa,pb=pts[q[a]],pts[q[b]]
            if pa and pb: line(img,w,h,pa[0],pa[1],pb[0],pb[1],(255,0,0))
    writepng(os.path.join(OUT,"ctl_cut%d.png"%ci),w,h,img)
    print("cut%d done"%ci)
