...
#include <limits.h>
#include "re15_bss.h"
size_t g_vlc_src_used = 0;
int g_vlc_bitcount = 0;
int g_vlc_stop_after_mb = 0;
int g_vlc_mb_done = 0;
                        ...
                        int16_t *dst, size_t dst_capacity)
{
    ...
    while (s.dst_offset < s.dst_capacity) {
        if (g_vlc_stop_after_mb && blocks >= g_vlc_stop_after_mb*6) break;
        blocks++;
        ...
        while (1) {
            if (!write_coeff(&s, code)) { g_vlc_src_used=s.src_offset; g_vlc_bitcount=s.bit_count; g_vlc_mb_done=blocks/6; return (int)s.dst_offset; }
            flush_bits(&s, VLC_BITS(code));

            int ac = decode_ac(&s);
            if (ac == VLC_DECODE_FAIL) { g_vlc_src_used=s.src_offset; g_vlc_bitcount=s.bit_count; g_vlc_mb_done=blocks/6; return -99; }
    ...
    g_vlc_src_used = s.src_offset; g_vlc_bitcount = s.bit_count;
    g_vlc_mb_done = blocks/6;
