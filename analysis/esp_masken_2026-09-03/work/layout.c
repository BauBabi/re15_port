...
int main(int argc, char**argv)
{
    for (int ai=1; ai<argc; ai++) {
        ...
        FILE*f=fopen(path,"rb"); if(!f){ printf("{\"f\":\"%s\",\"err\":\"open\"}
",path); continue; }
...
",path,fsz); fclose(f); continue; }
        for (long ci=0; ci<nch; ci++) {
            ...
            /* Brute-Force-Scan (nur wenn die Vorhersage NICHT traegt): jedes 4-Byte-Offset */
            ...
            long trailerP=-1; uint32_t trailerFlag=0;
            long refoff = sz_pred ? pred : scan_off;
            if (refoff>=0) {
                for (long P=8;P<=0x10000;P+=4) {
                    uint32_t v; memcpy(&v,buf+P-8,4);
                    if ((long)v==refoff) { uint32_t fl; memcpy(&fl,buf+P-4,4); trailerP=P; trailerFlag=fl; break; }
                }
            }
            /* Bytes direkt nach dem VLC-Ende (Marker-Suche) */
            unsigned char pb[16]; memset(pb,0,16);
            if (vlcend>=0 && vlcend+16<0x10000) memcpy(pb, buf+((vlcend/4)*4), 16);
            printf("{\"f\":\"%s\",\"c\":%ld,\"id\":%u,\"rlw\":%u,\"q\":%u,\"ver\":%u,\"wrote\":%d,\"bits\":%ld,\"vlcend\":%ld,"
                   "\"pred\":%ld,\"sldsize\":%u,\"sldend\":%ld,\"scanoff\":%ld,\"scansz\":%u,\"scanend\":%ld,\"P\":%ld,\"flag\":%u,"
                   "\"post\":\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\"}
",
