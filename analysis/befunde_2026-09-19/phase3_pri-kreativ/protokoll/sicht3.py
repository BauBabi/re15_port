...
EXE_DIR = os.path.join(WURZEL, "re15_port", "build_p3", "platform", "pc")
...
def _weg(pfad):
    """befund.log loeschen — der vorige Spiellauf kann die Datei noch offen haben
    (WinError 32). Mehrfach versuchen statt abzubrechen."""
    import time as _t
    for _ in range(20):
        if not os.path.exists(pfad):
            return True
        try:
            os.remove(pfad); return True
        except OSError:
            _t.sleep(1.0)
    return False
    ...
    while time.time() - t0 < 240:
        ...
        if os.path.exists(bl):
            for ln in open(bl, errors="replace"):
                ...
                if m and m.group(2) == room[4:] and int(m.group(1)) > TP_FRAME - 390:
...
def kandidaten(room, cut, n=6, nur_band=None):
    ...
    for band, pts in floor.items():
        ...
        for (wx, wz) in pts:
            ...
            if not c.any():
                continue
            # Toleranz = ein OT-Bucket (65536/1023, ZSF3 = 341 @0x80066c70) wie in der Abnahme
            B = geometrie.BUCKET
            nv = int((ref[c] < sv[c] - B).sum()); nh = int((ref[c] > sv[c] + B).sum())
            px = int(box.sum())
            if nv and not nh:
...
def lauf(room, cut, wx, wz, tag, cut_env=None):
    dump = os.path.join(WURZEL, "build", "p3", "sicht", "%s_C%d_%s" % (room, cut, tag))
