#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Textured contact sheet of every distinct RE1.5 room-prop geometry."""
...
rows = [r for r in json.load(open(os.path.join(WORK, 'props.json'))) if r['ok']]
seen = {}
for r in rows:
...
def rank(lst):
    ...
    ok = 1 if 200 <= dy <= 1200 and 50 <= max(dx, dz) <= 420 else 0
    return (0 if ok else 1, rnd - (dy / float(max(dx, dz))) * 0.05)
...
def rdt(stage, room):
    k = (stage, room)
    if k not in cache:
        cache[k] = open(os.path.join(ROOT, stage, room + '.RDT'), 'rb').read()
    return cache[k]
...
print('distinct RE1.5 props: %d' % len(reps))
...
for pg in range(npage):
    ...
    for k in range(PER):
        ...
        dr.text((cx + 2, cy + 14), 'dx%d dy%d dz%d %s' % (
            r['dx'], r['dy'], r['dz'],
            ('tim%d %dx%d' % (tim['bpp'], tim['w'], tim['h'])) if tim else 'noTIM'),
            fill=(150, 200, 230), font=F)
    fn = os.path.join(OUTD, 're15_tex_page%02d.png' % pg)
    sheet.save(fn)
    print('wrote', fn)
with open(os.path.join(OUTD, 'index15.txt'), 'w') as f:
    for gi, lst in enumerate(reps):
        r = lst[0]
        f.write('#%03d dx=%d dy=%d dz=%d faces=%d n=%d  %s\n' % (
            gi, r['dx'], r['dy'], r['dz'], r['nfaces'], len(lst),
