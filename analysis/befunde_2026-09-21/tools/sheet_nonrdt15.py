...
def is_rdt(f):
    u = f.upper()
    return (u.startswith('PL0/RDT/') and (u.endswith('.RDT') or '/OBJ/' in u)) or (u.startswith('STAGE') and u.endswith('.RDT'))
...
for pg in range(npage):
    ...
    fn = os.path.join(OUTD, 're15_nonrdt_page%02d.png' % pg)
    sheet.save(fn)
    print('wrote', fn)
with open(os.path.join(OUTD, 'index_re15_nonrdt.txt'), 'w') as f:
