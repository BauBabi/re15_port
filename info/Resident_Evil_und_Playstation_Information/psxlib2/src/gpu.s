...
# src for dot primitive
# Jum Hig 1999
###############################################################################
# Added by Jum Hig 1999   !!! needs checking
	.globl	GPU_SortDot					# void GPU_SortDot(GsOT *orderTable, PsxUInt32 pos,
	.ent	GPU_SortDot					#                         DOT *dot);
GPU_SortDot:							#
	LW		$4, 0x10($4)				# {
	...
	SW		$31, 0x10($29)			#
								#
	ADDU	$3, $5, 0x0c					#     Dot takes 12 bytes (i hope)
	SW		$3, CmdBufferAddress			#     /* Get ready for next prim */
								#
	LW		$8, 0x0($6)				#     dot.color >--+
	LW		$9, 0x4($6)				#     dot.vert  >--+-+
								#                  | |
	SW		$8, 0x4($5)				#     <------------+ |
	SW		$9, 0x8($5)				#     <--------------+
	...
	SB		$2, 0x3($5)				#     Length of the cmd
								#
	ADDI	$2, $0, 0x68					#     Fill in the primitive cmd for a dot
								#
...
# src for rect8 primitive (8x8 rectangle, single colour)
# (similar to dot primitve and rect16 primitive)
# Jum Hig 1999
###############################################################################
# Added by Jum Hig 1999   !!! needs checking
	.globl	GPU_SortRect8					# void GPU_SortRect8(GsOT *orderTable, PsxUInt32 pos,
	.ent	GPU_SortRect8					#                         DOT *dot);
GPU_SortRect8:						#
	LW		$4, 0x10($4)				# {
	AND		$5, $5, 0xFFFF			#     /* Resolve $4 and $5 down to where */
	SLL		$5, $5, 0x2				#     /* we'll insert the ordertable elemnt, */
	...
	SB		$2, 0x3($5)				#     Length of the cmd
								#
	ADDI	$2, $0, 0x70					#     Fill in the primitive cmd for a rect8x8
								#
								#     Right, we've built it, just need
								#     to link it in to the table
	JAL		Gpu_InsertOrderTableElement		#
...
# src for rect16 primitive (16x16, single colour)
# (similar to dot primitive)
# Jum Hig 1999
###############################################################################
# Added by Jum Hig 1999   !!! needs checking
	.globl	GPU_SortRect16				# void GPU_SortRect16(GsOT *orderTable, PsxUInt32 pos,
	.ent	GPU_SortRect16				#                         DOT *dot);
GPU_SortRect16:						#
	LW		$4, 0x10($4)				# {
	AND		$5, $5, 0xFFFF			#     /* Resolve $4 and $5 down to where */
	SLL		$5, $5, 0x2				#     /* we'll insert the ordertable elemnt, */
	...
	SB		$2, 0x3($5)				#     Length of the cmd
								#
	ADDI	$2, $0, 0x78					#     Fill in the primitive cmd for a rect16x16
								#
								#     Right, we've built it, just need
								#     to link it in to the table
	JAL		Gpu_InsertOrderTableElement		#
...
# src for 8x8 sprite primitive
# Jum Hig 1999
###############################################################################
# Added by Jum Hig 1999   !!! needs checking
	.globl	GPU_SortSprite8			# void GPU_SortSprite8(GsOT *orderTable, PsxUInt32 pos,
	.ent	GPU_SortSprite8					#                    SPRITE *sprite);
GPU_SortSprite8:							#
	LW		$4, 0x10($4)				# {
	AND		$5, $5, 0xFFFF			#     /* Resolve $4 and $5 down to where */
	SLL		$5, $5, 0x2				#     /* we'll insert the ordertable elemnt, */
	...
	SW		$31, 0x10($29)			#
								#
	ADDU	$3, $5, 0x10					#     Sprite8 takes 16 bytes (i hope)
	SW		$3, CmdBufferAddress			#     /* Get ready for next prim */
								#
	LW		$8, 0x0($6)				#     sprite.color >----+
	LW		$9, 0x4($6)				#     sprite.vert  >----+-+
	LW		$10, 0x8($6)				#     sprite.clut/tex >-+-+-+
								#                       | | |
	SW		$8, 0x4($5)				#     <-----------------+ | |
	SW		$9, 0x8($5)				#     <-------------------+ |
	SW		$10, 0xc($5)				#     <---------------------+ 
	...
	SB		$2, 0x3($5)				#     Length of the cmd
								#
	ADDI	$2, $0, 0x74					#     Fill in the primitive cmd for a sprite8
								#
								#     Right, we've built it, just need
								#     to link it in to the table
	JAL		Gpu_InsertOrderTableElement		#
...
# src for 16x16 sprite primitive
# Jum Hig 1999
###############################################################################
# Added by Jum Hig 1999   !!! needs checking
	.globl	GPU_SortSprite16			# void GPU_SortSprite16(GsOT *orderTable, PsxUInt32 pos,
	.ent	GPU_SortSprite16					#                         SPRITE *sprite);
GPU_SortSprite16:							#
	LW		$4, 0x10($4)				# {
	AND		$5, $5, 0xFFFF			#     /* Resolve $4 and $5 down to where */
	SLL		$5, $5, 0x2				#     /* we'll insert the ordertable elemnt, */
	...
	SB		$2, 0x3($5)				#     Length of the cmd
								#
	ADDI	$2, $0, 0x7c					#     Fill in the primitive cmd for a sprite16
								#
								#     Right, we've built it, just need
								#     to link it in to the table
	JAL		Gpu_InsertOrderTableElement		#
