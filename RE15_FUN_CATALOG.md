...
| 0x8003fbe8 (PSX 0x18 = GOSUB) | **GOSUB** (sub_id@pc[1], save ret-PC thread+0x144, redirect PC→`DAT_800b3f70`[sub_id] = **sub_scd table base**). ✅ Engine `op_gosub` CORRECT (w71w6amhp 2026-05-29). Earlier "message handler" label = misread `Evt_exec(0x04)` arg byte. | ghidra1:149773-149799 | high |
...
| 0x80042bac | Door_aot_set AOT scan: iterate 0x94-byte AOTs (DAT_800ac9b0), filter type/floor/flag, FUN_80014368 (flag0x40) or FUN_80042b64 (AABB) | ghidra1:153690-153999 | high |
