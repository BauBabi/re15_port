...
| FUN_80101b64 | **Zombie approach-state handler** (a PTR_FUN_801217a0 entry). Transitions ent+0x4: timer ent+0x9c-- ==0 → 0x101; **`(ent+0x1d0 < 2000) && (FUN_8001a9cc(player,0x2c8)!=0)` → 0x701**; far/flee (FUN_8001a9cc(...,0x5f4)==0 && ent+0x1d8&0x10 && dist>10000 && rng&7==0) → 0x801 (else 0x201). ⚠️ The agent label "0x701==attack" is UNCONFIRMED/likely-wrong: 0x701 fires when the player is close AND OUTSIDE the ±0x2c8 front arc → reads as turn-to-face, not lunge. The actual lunge state + the action→FUN_80017fa4 path NOT yet mapped | STAGE1_full/FUN_80101b64.c | med |
