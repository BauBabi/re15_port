...
| zombie FSM (3-level hierarchical, RAM-read HASH_1.sav + handler trace) | **NESTED dispatch** (the earlier "flat 22-entry table" was a misread): (1) MAIN state = entity+0x4 byte → PTR_FUN_801217a0[+0x4] (only ~5 entries: 0=FUN_8011d84c INIT, 1=FUN_8011d9f4, 2=0x8011db40, 3=0x8011db88, 4=0x80050be8 EXE). (2) FUN_8011d9f4 (state 1) SUB-dispatches PTR_FUN_801217b4[(entity+0x24)&0xf] (= 0x801217a0+0x14; the "entries 5-21" the agent saw are THIS sub-table: 0x8011da48, then shared EXE sub-behaviors 0x8004f1xx-0x80051xxx). (3) ANIM table PTR_FUN_80120208[+0x4] (separate, FUN_8011d6d4/FUN_8010a8c8 drive it). entity ptr is word* so `ent[1]`=byte+0x4 (state), `ent[0x62]`=byte+0x188 (model-pool), `ent[0x6a]`=+0x1a8=&player(DAT_800aca54). INIT (FUN_8011d84c) stores the player ptr + writes AI params into the model-pool sub-struct (+0x5fc=**0x2c8** the attack arc, +0x5f8=0x40,+0x5fa=0x30,+0x5fe=0x138). Stationary zombie = state 4. ⚠️ the LUNGE main-state+sub-state still not pinned (3-level trace over ~16 sub-behaviors, OR one mid-lunge savestate reads live +0x4 AND +0x24) | HASH_1.sav RAM + STAGE1_full | high |
