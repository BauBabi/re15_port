...
- **STATUS 2026-07-07 (2 von 3 Teilen ERLEDIGT + getestet, Commits 395a8407/a4aa986e/b89ada17):**
  - ✅ **(a) Edge-Entfernung** (aot_common.c fire-Ternary): AUTO-Zone feuert jeden Frame inside. Die AUTO_EVENT-Gefahr (`scd_event_fire` ohne Running-Guard → N Sub-Kopien) STATISCH aufgelöst: Frame-Reihenfolge scd_tick→scan→fire + der Sub self-disabled auf Opcode 0. VERIFIZIERT via `re15_rdt_parse`-Dump: ROOM1150 sub08 (Irons) = `46 06 00…` = Aot_reset(6,0) als Opcode 0 → genau 1 Fire (inhärent: das Original hat auch keinen Edge, also self-disablen alle shippenden AUTO-Subs zuerst). `op_aot_reset` sce==0 → disable verifiziert.
  - ✅ **(c) Band-Gate** (aot_common.c + op_aot_set): generisch für Event-AOTs (GENERIC/AUTO_EVENT/MESSAGE/EXAMINE), `a->band` aus Aot_set pc[4] befüllt, Gate `(band&0x80)||player_band==band` nur bei pb>=0 (wie der Door-Gate). Unit-getestet (test_aot_edge §4).
  - 🔶 **(b) Item/NPC-Entity-Scan** (RE'd, noch nicht portiert): FUN_800436a8 scannt **Player 1× + Item-Loop (0x800b0000-Region) + NPC-Loop (Count @0x800ac778)**, jeder Aufruf = FUN_80042bac(entity a0, class a1 ∈ {1,2,4}, param a2=0); FUN_80042bac berechnet den Forward-Reach der Entity (rot @+0x6a via FUN_8004f008, pos +0x34/+0x3c → sp+24/+32) und testet die AOTs mit dem Class-Mask-Gate. Port-Fix: ein AOT-Class-Mask-Feld (aus dem Flagbyte) + re15_aot_scan über die Item/NPC-Actor-Pools. ADDITIV (bricht keine Player-AOTs) aber nischig — in den STAGE1-Testräumen inert (alle AOTs Player-only). test_aot_edge ist das Harness dafür.
- **Regressions-Test:** `re15_port/tests/unit/test_aot_edge.c` (Edge-every-frame + Band-Gate). 52/52 ctest; Intro + ROOM1130/1140/1150 booten clean.
...
- **STATUS 2026-07-07 — Kopplung AUFGELÖST + Hit-Flinch PORTIERT (Commit c35ecd95):** Der Defer-Grund („Damage-Trigger deferred → state nie 2/3") gilt NICHT MEHR — die diese Session portierte Gegner-KI wendet Schaden an (`pl->hp -= N` + `hit_react`). Der beobachtbare Teilschritt (Hit-Flinch) ist verdrahtet: game_step erkennt einen nicht-tödlichen hp-Drop (s_prev_hp pre-damage aktualisiert → der Drop aus re15_enemy_ai_run_all am Step-Ende wird nächsten Tick gesehen) und rootet den Spieler im Hurt-Clip **0xa** (byte-true +0x94=0xa @0x80035bd8) für dessen Dauer → zurück zu Idle. Neue else-if-Branch wie stair/grab/dead (engine-driven skip, kein Pad), RVD-Cam-Scan bleibt. Gegated auf plain gameplay (grabbed/dead/stair/aim haben Vorrang). **DEFERRED/faithful-line:** das exakte 4-Richtungs-Hit-Sub-FSM (DAT_800aca59 → 0x80035b70/c a8/de0/f64), der Aim-Interrupt, die exakte Clip-0xa-Dauer. **Verifikation:** 52/52 ctest; ROOM1240-Intro NULL Spurious-Flinch (hp stabil 100); ROOM1140-Grab-Combat unberührt (grabbed-Gate). Positives Feuern logik-korrekt aber nicht headless beobachtbar (Gegner treffen den stationären Spieler ohne Provokation nicht — Harness-Limit, [[reai-v2-savestate-repacker]]).
