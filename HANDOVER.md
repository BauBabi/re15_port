...
**ESP-KORREKTUR (WICHTIG, dieser Commit):** Was die Vorsessions „ESP-Effekt-Script-VM“ nannten (`FUN_8003f0a0` + die `@0x800744a8`-Opcode-Tabelle + der 0x170-Instanz-Pool), **IST die SCD-VM** — kein separates Effekt-Subsystem. Beweis: der 0x170-Pool wird aus `RDT+0x40/+0x44` (main/sub SCD) geseedet (`FUN_8003ef6c`→`FUN_8003ee3c`), das SCD-Format (`body=start+u16[start]`) = das `FUN_8003edec`-pc-Seed, und `@0x800744a8[0x4c]` = `op_sce_espr`. **Der Port hat diese VM BEREITS: `re15_port/engine/src/scd_vm.c`** (87 Opcode-Mappings, im Spiel verdrahtet — spawnt die Zombies, fährt Combat).
- **Ich hatte (dem falschen Handover-Framing folgend) eine PARALLELE byte-true VM `re15_esp_vm.c` gebaut (45 Ops) — Duplikat-Arbeit.** Auf Nutzer-Entscheidung KONSOLIDIERT: `re15_esp_vm.{h,c}` + `test_esp_vm.c` ENTFERNT; `scd_vm.c` bleibt die eine SCD-VM. (33/33 ctest grün nach Entfernung.)
- **CROSS-CHECK-ERTRAG (der bleibende Wert):** meine byte-true RE deckte einen Bug in scd_vm.c's „disasm-verified“ Längen-Tabelle auf: **`s_opcode_sizes[0x3D]` war 4, byte-true ist 3** (`LAB_80041238` `addiu pc,pc,3`; unmapped → `op_unknown` desynct +1 bei jedem 0x3d). **In scd_vm.c GEFIXT.** (0x15 war ein Fehlalarm — gemappt zu `op_default`, Tabellenwert ungenutzt.)
- **ÜBERLEBT als Referenz:** `RE15_SCD_OPCODES_REFERENCE.md` — byte-true Katalog aller 95 SCD-Opcodes (`@0x800744a8`-Adressen, Operanden, pc-Advances, Semantik). Nützlich, um scd_vm.c weiter zu validieren (Handler-Semantik Op-für-Op vs. die byte-true RE).
- **SEPARAT von der SCD-VM (NICHT entfernt): ESP-A/B** in `re15_esp.c` — der RDT-EFF/TIM-**Parser** (RDT+0x4C) + der Effekt-Sprite-**Pool** (`DAT_800b2360/2368`, AABB-Cull `FUN_8004d5f0`). Daten-/Pool-Schicht, kein VM-Duplikat (`test_esp_parse` grün).
- **✅ CROSS-CHECK VERTIEFT (dieser Commit) — scd_vm.c-Handler-Semantik vs. byte-true RE (16 überlappende Ops via Workflow, jede Divergenz selbst gegen die Bytes verifiziert):** 14 MATCH, 2 DIVERGE. **(1) 0x35 `op_member_set2` = ECHTER BUG, GEFIXT:** las den Wert aus `t->locals[]` (per-Thread-u8-Platzhalter) statt aus `g_scd.work_vars[pc[2]]` (global s16 = `DAT_800b0fd0`; byte-true `lh @0x80041138`) → falsche Quelle/Breite/Range; das Lese-Gegenstück 0x3E las schon korrekt aus work_vars. Fix nutzt dasselbe Muster wie op_cmp/op_member_cmp. **(2) 0x0d `op_for` = BEKANNTE defensive Abweichung (KEIN Fix):** bei count==0 überspringt der Port den Block (0 Iter), byte-true würde count=0 pushen → Next underflowt zu 65535 Iter (= Hang-Risiko); die Port-Wahl ist verteidigbar + dokumentiert. count>0 = MATCH. (Sekundär: Port deckelt Loop-Nesting auf <4.)
- **✅ CROSS-CHECK PASS 2 (dieser Commit) — 35 weitere scd_vm.c-Handler vs. byte-true (Workflow, jede DIVERGE selbst verifiziert):** 17 MATCH, 8 STUB, **10 DIVERGE**. **GEFIXT (2 saubere Flag-Check-Prädikate):** **0x51 `sce_key_ck`** (No-Key-Return war invertiert: gab `param` statt byte-true `param^1` → IF-Blöcke in Cinematics nahmen den falschen Zweig) · **0x52 `sce_espr_control`** (war `return 1`-Stub; byte-true ist ein Flag-AND-Prädikat = `param^1` bei ungemodelltem DAT_800ac76c). 33/33 grün.
- **✅ 0x4b GEFIXT (dieser Commit) — der fehlende Live-Cut-Switch:** byte-true Tail (`LAB_80040414` @0x800404ac) — nach dem Zone-Relabel: **wenn der aktive Cut == a, sofort auf b umschalten** (die in-room-Live-Ersetzung, die der ROOM1130-Nutzer als fehlend sah). Nutzt den Port-Cam-Change-Pfad (`g_scd.cam_id`+`cam_change_pending`); aktiver Cut aus `g_re15_active_cut`. + Unit-Test. **= 6 Live-VM-Bugs gefixt.** (0x53 untersucht, aber NICHT gefixt — Multi-Mode, mode-1-Objekt unbekannt, modes 3/5 nicht darstellbar, mode-2-Konvention unverifiziert → raten-verboten, deferred.)
- **✅ 0x58 GEFIXT — RE2-Fehlattribution korrigiert:** byte-true `LAB_8003fd54` ist ein **Flag-Bank-Bit-Check-Prädikat** (Bank=`pc[1]`, Word=`work_vars[desc&0xff]>>5`, Bit=`desc&0x1f` MSB-first, Invert wenn `(desc>>8)==0`), NICHT `plc_rot`. Reimplementiert als `op_flag_ck2` (nutzt das byte-identische `re15_game_flag_get`) + **3 Unit-Tests** in `test_scd_opcodes` (Read/Invert/Word-aus-work-var). **= 5 Live-VM-Bugs gefixt** (0x3d, 0x35, 0x51, 0x52, 0x58).
- **⚠️ GROSSER BEFUND — scd_vm.c hat noch RE2-FEHLATTRIBUIERTE Handler (laufen das FALSCHE für RE1.5), brauchen Reimpl + externe Tabellen:** **0x47** (byte-true = Object-Handler-Dispatch via `DAT_800ac9b0`-Registry; Port macht `aot_on`) · **0x50** (byte-true = Bytecode-Ptr-Latch in `DAT_800ac9b0`; Port macht `item_aot_set`) · **0x53** (byte-true = indirekter Work_set-Bind `inst+0x154`; Port stubt als `fade_set`). **Fehlende Seiteneffekte:** 0x2d (Port fabriziert Kollision aus Model-Daten-Bytes) · 0x38 (Stub; byte-true setzt eine SCA-Kollisions-AABB) · 0x4a (Port lässt die Entity-state-4-Writes via inst+0x154 weg) · 0x4b (Port lässt den sofortigen Live-Cut-Switch weg). Volle Liste: `RE15_SCD_OPCODES_REFERENCE.md`.
- **➡️ NÄCHSTER SCHRITT (offen, braucht Richtung):** (a) **0x53** (Work_set-Bind, wie op2e — der Port hat `op_work_set`/`work_slot` schon, evtl. sauber fixbar); (b) die **fehlenden Seiteneffekte** (0x4a Entity-Writes, 0x4b Cut-Switch); (c) **0x47/0x50** (brauchen das `DAT_800ac9b0`-Object-Registry — tiefer); (d) **C3-Render** · (e) anderes Thema. ⚠️ Jeder Fix ändert Live-Verhalten — byte-true verifiziert + getestet.
...
> ⚠️ **TEILWEISE ÜBERHOLT — siehe SECTION 0 „ESP-KORREKTUR“.** „ESP-C = eine separate Effekt-Script-VM“ war FALSCH: `FUN_8003f0a0`/`@0x800744a8` IST die SCD-VM (= `scd_vm.c`). Der hier unten beschriebene „ESP-C-VM-Port“ (`re15_esp_vm.c`) wurde als Duplikat ENTFERNT; die byte-true Opcode-RE überlebt in `RE15_SCD_OPCODES_REFERENCE.md`. **ESP-A/B (`re15_esp.c`, der Parser + Pool) bleiben gültig** — nur der „ESP-C-VM“-Teil dieser Notiz ist überholt.
