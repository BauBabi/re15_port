# RE1.5 Port — Session-Handover (Stand 2026-06-30, Phase 8.16 + ESP-A: Player-Raise-Clip + ESP-Sprite-Parser byte-true; Combat spielbar)
...
- **✅ ESP-C2 BATCH 1 ERLEDIGT (dieser Commit), 34/34:** die **Control-Flow-Kern-Opcodes** `@0x800744a8[0x00..0x08]` + Nop-Aliase, byte-true disasm-verifiziert (`@0x8003f1c0..f3dc`) und in `re15_esp_vm.c` portiert + registriert in `re15_espvm_reset()`; Test-Teil (7) treibt jeden mit echtem Bytecode. Instruktionssatz: **0x00/0x1c/0x1d–0x20** nop-continue · **0x01** end/return-level (Level-0→deaktiv+STOP, sonst Level-Pop→CONT) · **0x02** wait-1-Frame · **0x03** jump-script (`FUN_8003edec(id@pc+3)`) · **0x04** spawn (`FUN_8003ee3c(slot@pc+1,id@pc+3)`) · **0x05** kill-instance[idx@pc+1] · **0x06** loop-push (push `pc+4+u16@pc+2`, depth++) · **0x07** loop-jump (`pc+=u16@pc+2`, depth--) · **0x08** loop-pop (`pc+=2`, depth--). Synthetische C1-Test-Ops auf 0xF0+ verschoben (Kollision mit echten Ops vermieden).
- **✅ ESP-C2 BATCH 2 ERLEDIGT (dieser Commit), 34/34:** die per-Level-Loop-Counter-Ops **0x09–0x0c** byte-true (disasm-verifiziert `@0x8003f3e0..f53c`), Test-Teil (8). **0x09** counter-set (`cidx++`, speichert `u16@pc+2` in Counter-Array `@inst+level*8+0xa0`, `pc+=1`) · **0x0a** counter-wait (`counter[cidx]--`; ≠0→STOP/bleibt, ==0→`pc+=3`+`cidx--`) · **0x0b** counter-push (`cidx++`, `pc+=1`) · **0x0c** wait-condition (Gate `FUN_8004efe4(0x800aca3c,0x1a)`=MSB-first-Bit5 `||` `*0x800b8520&0x80`; Port-Gate `re15_espvm_set_wait_gate`, Default 0=Normalspiel). **Schlüssel-Befund: `0x09`+`0x0a` zusammen = „warte N Frames"** (`[09 0a lo hi]`: op9 liest count@+2, opa zählt pro Frame runter → `pc+=3` über op+count). Damit ist der **strukturelle VM-Kern komplett (Ops 0x00–0x0c)**.
- **✅ ESP-C2 BATCH 3 ERLEDIGT (dieser Commit), 34/34:** das **2. FOR-Loop-Subsystem** (Ops 0x0d–0x1a) byte-true (disasm-verifiziert `@0x8003f540..fcf0`), Test-Teil (9) inkl. echter 3-fach-Schleife. Es indiziert über `counter-index inst[level+8]` (ci) vier per-Level-Arrays: **DEPTHARR** `@+0x0c+level*4+ci` (byte), **PCARR** `@+0x20+level*0x10+ci*4` (body-start u32), **TGTARR** `@+0x60+level*0x10+ci*4` (end-target u32), **CARRAY** `@+0xa0+level*8+ci*2` (count u16, geteilt mit 0x09/0x0a). Ops: 0x0d FOR-begin(count) · 0x11 FOR-begin(no-count) · 0x0e NEXT(loop→PCARR/pop) · 0x10 loop-back · 0x1a break→TGTARR+restore-depth · 0x16 pop-ci · 0x14 skip6 · 0x15 skip2 · 0x17 block-setup(depth/ci/sp+rel-jump) · 0x19 pop-level(←saved-pc +0x144). **→ der gesamte VM-Control-Flow (0x00–0x1a) ist komplett.**
- **✅ ESP-C2 BATCH 4 ERLEDIGT (dieser Commit), 34/34:** die zwei rein instanz-lokalen Field-Ops **0x2f** (u16→`inst[0x158+id*2]`, 4-byte) + **0x31** (per-Frame-Integrator: 6× `inst[0x158..0x162] += inst[0x164..0x16e]` = Position+Velocity, genau die 6 Words die der Allokator @+0x158 nullt). **WICHTIG:** die Workflow-Klassifikation hatte auch 0x32/0x4d als `portable` gelabelt — **Selbst-Disasm widerlegte das** (0x32 schreibt durch `inst[0x154]`-Work-Pointer, 0x4d in den Effekt-Pool `DAT_800b2368`) → korrekt deferred. Damit sind die rein instanz-lokalen Opcodes ERSCHÖPFT (Control-Flow 0x00–0x1a + 0x2f/0x31 = **25/95 portiert**).
- **✅ ESP-C2 BATCH 5 ERLEDIGT (dieser Commit), 34/34:** das **VM-Register-Subsystem** `DAT_800b0fd0` (256× u16, als Port-Static `s_reg`, beim Reset genullt) + die 6 Ops byte-true (disasm-verifiziert): **0x24** reg=s16-imm · **0x25** reg=reg · **0x23** compare (7 Selektoren `== > >= < <= !=` + `&`-Bittest; **returnt CONT/YIELD = der IF-Mechanismus**) · **0x26/0x27** arith via `FUN_80040140` (12-Wege: add/sub/mul/div/mod/or/and/xor/not/shl/shr-log/shr-arith, byte-true inkl. div-by-0-Guard) · **0x1b** FOR-begin mit count aus Register. Test-Teil (11) exhaustiv: alle 12 Arith-Ops + alle 7 Compare-Selektoren (true+false) + Register-FOR-Loop. **31/95 Opcodes portiert.**
- **✅ ESP-C2 BATCH 6: GOSUB `0x18`** byte-true (@0x8003fbe8) — saved-pc→`+0x144[level]`, level++, neuer Level reinit, jump zu `offset_table[id@pc+1]`. Partner zu op19/op1-RETURN.
- **✅ ESP-C2 BATCH 7 (dieser Commit): die CONDITIONAL-Evaluatoren `0x0f`/`0x12`/`0x13`** byte-true (disasm-verifiziert @0x8003f6f4/f930/fa5c), Test-Teil (12). **0x12** = compound-IF (AND/OR-Prädikat-Kette: dispatcht Sub-Prädikate via `table[subop]`, kombiniert AND wenn inter-pred-u16-flag==0 / sonst OR → TRUE: `pc=PCARR[ci]`/loop-back, FALSE: `ci--`/exit). **0x0f** = IF/while-block-begin (FOR-Frame-Setup + dieselbe Prädikat-Kette → TRUE: Block betreten, FALSE: `pc=TGTARR[ci]`+`ci--`). **0x13** = register-`switch` (`switch=DAT_800b0fd0[reg]`; Case-Liste mit Markern 0x15=end/0x16=default; Match→Case-Body, sonst skip). Prädikate = op23 (returnt 1/0). **Der gesamte VM-Control-Flow inkl. Conditionals/Switch ist KOMPLETT** (Loops×2 + Counter + Register + Compare + GOSUB + IF/WHILE/SWITCH). **35/95 portiert.** Geteilter `espvm_eval_chain`-Helfer. (Hinweis: über un-portierte Flag-Test-Prädikate [Stub returnt STOP=2] wären die Conditionals nicht byte-true — gilt nur für op23-Prädikate.)
- **✅ ESP-C2 BATCH 8 (dieser Commit): die WORK-STRUCT-Foundation `0x2e`/`0x32`/`0x33`/`0x42`** byte-true (disasm-verifiziert @0x80040d2c/41048/41080/41f88) — **die Effekt-VM pokt jetzt echte Spiel-Entities**. **op2e** = Binder: `inst[+0x154]` ← Ziel je `kind@pc+1`: **kind 1=Player** (`0x800aca54`), **kind 2=Entity-Liste** (`0x800acc2c + idx*0x1f4`), kind 3=Effekt-Sprite-Pool (`0x800b3f98 + idx*0x94`), kind 5=Pointer-Tabelle (`0x800b23f4`). Port: `inst->bound_slot` = g_actors-Slot (kind1→0, kind2→idx; kind3/5 deferred = -1); op2e nullt zusätzlich `inst[0x158..0x16f]`. **Setter** (schreiben durch `bound_slot` → `g_actors[]`): **op32** pos x/y/z (entity+0x34/38/3c, 3×s16), **op33** rot x/y/z (entity+0x68/6a/6c, 3×u16), **op42** state-init (entity+0x4=1, +0x5/6/7=0). **39/95 portiert.** Test-Teil (13). `re15_esp_vm.c` koppelt nun an `re15_actor.h`/`g_actors`.
- **✅ ESP-C2 BATCH 9 (dieser Commit): die MEMBER-SET-Setter `0x34`/`0x35` + `0x43`** byte-true (disasm-verifiziert @0x800410b8/41108/41fb8). **GROSSER BEFUND:** `FUN_8004116c` (die 20-Member-id→Offset-Dispatch-Tabelle, die 0x34/0x35 nutzen) ist **byte-identisch zur bestehenden `re15_actor_set_member`-API** (member 0=x@+0x34 … 16=anim_flags@+0x1c4 … 19=hp@+0x1ba) → op34/op35 mappen DIREKT darauf. **op34** member-set(member@pc+1, s16-imm@pc+2, pc+=4) · **op35** member-set(member@pc+1, reg[idx@pc+2], pc+=3) · **op43** anim_flags(+0x1c4=member16) SET/OR/XOR (sub-mode@pc+1). **Das löst auch die mehrdeutige +0x1c4 auf = `anim_flags`.** **42/95 portiert.** Test-Teil (13f/g/h).
- **✅ ESP-C2 BATCH 10 (dieser Commit): die GET-FIELD-Ops `0x3d`/`0x3e`** byte-true (@0x80041238/41290). `FUN_80041358` (member-GET-Dispatch) ist **byte-identisch zu `re15_actor_get_member`**. **op3d** = `reg[idx@pc+1] = get_member(bound, sel@pc+2)` (pc+=3) · **op3e** = `compare(get_member(bound, sel@pc+2_lo), s16@pc+4)` per 7-Wege-Selektor@pc+2_hi → CONT/YIELD (= op23, LHS aus Entity-Feld). Geteilter `espvm_cmp7`-Helfer. **44/95 portiert.** Test (13i/j).
- **➡️ NÄCHSTER SCHRITT — Work-Struct an einer „nicht-raten"-Grenze:** **0x5a/0x5b (arith-auf-Feld) DEFERRED** — `FUN_80041554` liefert Feld-POINTER, `FUN_80040140` macht 16-bit-Arith → bei 32-bit-Feldern (x/y/z) würde `set_member` die oberen 16 bit überschreiben (Byte-true-Divergenz; braucht byte-genauen Feld-Akzessor). **0x3f/0x4a/0x5d DEFERRED** — schreiben `entity+0x1c8/+0x1ca` (NICHT in Member-Tabelle = unbekannt; raten-verboten). **Andere Gruppen:** Effekt-Pool (ESP-B, 0x2d/0x48/0x4c/0x4d) · Draw/GTE (C3, `FUN_80046a1c`) · Game-Flags (`@0x80074664`) · Audio 0x57/RNG 0x28/Spawn 0x44.
- **✅ ESP-C2 BATCH 11 (dieser Commit): der EFFEKT-POOL-SPAWN `0x4c`** byte-true (@0x80040858) — **die Effekt-VM spawnt jetzt einen ESP-B-Effekt-Sprite.** op4c IST das `op_sce_espr` (das `re15_esp.c` schon dokumentiert: `DAT_800b2368[slot]=pc+2`, count++, 18-Byte-Record) → mappt auf `re15_esp_spawn(type@+2,x@+4,y@+6,w@+8,h@+a)`, pc+=0x12. `re15_esp_vm.c` koppelt nun auch an `re15_esp.h`. **45/95 portiert.** Test (14). Andere Effekt-Pool-Ops deferred: **0x2d/0x48** = SEPARATER `DAT_800b3f98`-Render-Pool (stride 0x94 → Draw/C3), **0x4d** = Bytecode-Overlay-Slot (passt nicht zum Copy-Struct).
- **➡️ DIE SAUBER-PORTIERBAREN OPCODES SIND ERSCHÖPFT (45/95).** Alles Verbleibende braucht **C3/Render** (Draw/GTE + `DAT_800b3f98`-Pool + `FUN_80046a1c`) · ein **neues Subsystem-Modell** (Game-Flag-Bänke `@0x80074664`; Audio 0x57; RNG 0x28; Entity-Spawn 0x44) · oder verletzt **„nicht-raten"** (0x5a/0x5b, 0x3f/0x4a/0x5d). **Empfehlung: C3** (Bytecode-Quelle `DAT_800b3f70` + TIM/UV-LUTs + Render-Pfad) = der Weg zum *sichtbaren* Effekt, entsperrt die Draw-Ops.
- **📋 VOLLE OPCODE-KARTE: `RE15_ESP_VM_OPCODES.md`** — alle 95 Ops klassifiziert (Status/Kategorie/Calls/Summary) + Defer-Gruppen nach Abhängigkeit. Jeder PORTED-Eintrag selbst-disasm-verifiziert.
- **➡️ NÄCHSTER SCHRITT = die verbleibenden Defer-Gruppen (Reihenfolge in der Karte):** (2) **Work-Struct @inst+0x154** (0x32/0x33/0x34/0x35/0x3d/0x3e/0x3f/0x42/0x43/0x4a/0x5a/0x5b/0x5d — derefferenzieren einen Pointer auf ein extern alloziertes Anim/Render-Struct; erst das Struct binden); (3) **Effekt-Pool `DAT_800b2368`** (0x4c/0x4d/0x2d/0x48/0x37/0x38/0x4f → ESP-B-Integration); (4) **Draw/GTE** (0x36/0x3a/0x40/0x41/0x47/0x55 + `FUN_80046a1c`-SPRT → Render-Pfad/C3); (5) **Game-Flags/Cuts** (`DAT_800aca3c`/`@0x80074664`-Bänke); (6) Audio 0x57/RNG 0x28/Entity-Spawn 0x44/Engine-Helfer. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11.)
...
**→ NUTZER-WAHL: (A) ESP-Sprite-System bauen (siehe nächster Abschnitt).**

### ESP-SPRITE-SUBSYSTEM — Phase ESP-A ERLEDIGT; B/C/D als Nächstes
...
am echten Asset korrigiert habe (siehe ESP-A unten) — der **Pointer-Table-Eintrag ist ein ganzer Offset (entry+base), KEIN
offset/flags-Split**, und nur die Pointer-/TIM-Tabellen werden ABWÄRTS gelesen (das ID-Array vorwärts).
...
- **ESP-C (RENDER) — BLOCKER: braucht einen SAVESTATE eines aktiven Effekts (statischer Pfad erschöpft/verworren).**
  ...
  - **→ ESP-C PORT = ein dediziertes MEHR-SCHRITT-PHASE-PROJEKT** (wie die SCD-VM, aber für Effekte):
    ...
    - **🔄 (C2) IN ARBEIT — die ~52 Opcode-Handler `@0x800744a8` einzeln RE'n+porten** (volle Tabelle in `ghidra1_V2.txt` @0x800744a8,
      Einträge 0x8003f1d8.. bis 0x80040xxx; jeder Handler kriegt `inst` in a0 und gibt 1/2/0). **Batch 1 (0x00..0x08 + Nop-Aliase)
      ERLEDIGT** (Control-Flow-Kern: nop/end/wait/jump-script/spawn/kill/loop-push/jump/pop — siehe Instruktionssatz in SECTION 0).
      **Batch 2 (0x09–0x0c) ERLEDIGT** (per-Level-Loop-Counter `inst[level+8]`-Index + u16-Array `@inst+level*8+0xa0`;
      0x09 counter-set / 0x0a counter-wait [0x09+0x0a = „wait N frames"] / 0x0b counter-push / 0x0c wait-cond [Gate
      `re15_espvm_set_wait_gate`]) → der strukturelle VM-Kern 0x00–0x0c ist komplett. **Batch 3 = Draw/GTE/Field
      0x0d+** (0x0d/0x0f sind groß = die GTE/Sprite-Builder; ein Draw-Op = `FUN_80046a1c`).
...
- **ESP-D (MUZZLE-FLASH, das Nutzer-Ziel) — zuletzt:** `FUN_80045024` (Pistole = weapon a0>>24=2): **1-Frame**-Flash
  ...
  (`FUN_80045a64` Aim-Ray→Screen-Projektion, noch nicht portiert → faithful-line: Player-Screen-Pos + fixer Offset).
- **HINWEIS:** Room-ESP (`op_sce_espr`, FUN_80019354) und der Muzzle (`FUN_80045024`) sind ZWEI Einstiege in dasselbe
  Pool+Render-System; der Muzzle nutzt zusätzlich den g_entity-Model-Pool + den 0x801f-RAM-Scratch (runtime, via Savestate lesen).
