...
- **➡️ NÄCHSTER SCHRITT (Nutzer-Richtung):** (a) **Cross-Check vertiefen** — scd_vm.c-Handler-Semantik Op-für-Op gegen `RE15_SCD_OPCODES_REFERENCE.md` (weitere Bugs wie 0x3d finden); (b) **C3-Render** an scd_vm.c andocken (Draw-Ops/Sprites — fehlen scd_vm.c noch, nur length-stubs); (c) anderes Thema (Combat/Raum/Game-Over).
