...
- **⚠️ GROSSER BEFUND — scd_vm.c hat mehrere RE2-FEHLATTRIBUIERTE Handler (laufen das FALSCHE für RE1.5), brauchen Reimpl + externe Tabellen:** **0x47** (byte-true = Object-Handler-Dispatch via `DAT_800ac9b0`-Registry; Port macht `aot_on`) · **0x50** (byte-true = Bytecode-Ptr-Latch in `DAT_800ac9b0`; Port macht `item_aot_set`) · **0x53** (byte-true = indirekter Work_set-Bind `inst+0x154`; Port stubt als `fade_set`) · **0x58** (byte-true = `@0x80074664`-Flag-Bank-Bit-Check-Prädikat; Port macht `plc_rot`). **Fehlende Seiteneffekte:** 0x2d (Port fabriziert Kollision aus Model-Daten-Bytes) · 0x38 (Stub; byte-true setzt eine SCA-Kollisions-AABB) · 0x4a (Port lässt die Entity-state-4-Writes via inst+0x154 weg) · 0x4b (Port lässt den sofortigen Live-Cut-Switch weg). Volle Liste: `RE15_SCD_OPCODES_REFERENCE.md`.
- **➡️ NÄCHSTER SCHRITT (offen, braucht Richtung):** (a) die **RE2-fehlattribuierten Handler** byte-true reimplementieren — beginnend mit denen, deren externe Tabellen der Port schon modelliert (0x58 via dem Flag-Modell von `op_ck` 0x21); (b) die **fehlenden Seiteneffekte** ergänzen (0x4a Entity-Writes, 0x4b Cut-Switch); (c) **C3-Render**; (d) anderes Thema. ⚠️ Jeder dieser Fixes ändert Live-Verhalten — vorsichtig + getestet.
