...
- **➡️ NÄCHSTER SCHRITT = ESP-C2 BATCH 4 = die instanz-lokalen FIELD-SETTER** (per Workflow-Klassifikation `portable_now`): **0x2f** (u16→+0x158+id*2) · **0x31** (per-Frame-Integrator +0x164→accum) · **0x32** (3×s16→3 words) · **0x4d** (10-Byte-Block→Felder). Danach: die EXTERNEN/Draw/GTE/Spawn-Ops (brauchen C3 + Render-Pfad) — volle Klassifikation der 77 Ops liegt im Workflow-Output. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11.)
