...
- **➡️ NÄCHSTER SCHRITT = ESP-C2 BATCH 2:** die per-Level-Loop-Counter-Ops **0x09–0x0c** (`inst[level+8]`-Index + u16-Array `@inst+level*8+0xa0`; 0x0a = dec-and-branch, 0x0c liest `DAT_800aca3c`/`DAT_800b8520`), dann die **Draw-/GTE-/Field-Ops 0x0d+** (ein Draw-Op = der 40×30-SPRT `FUN_80046a1c` → PC-Sprite-Quad). Hook = `re15_espvm_set_opcode(idx, fn)`. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. **Mehr-Schritt-Projekt, NICHT in einem Rutsch.** (Statische Synthese hielt nicht; dynamischer Capture scheiterte — Briefing hat Messer w1, Muzzle eh 1-Frame.) Volle Karte: ESP-C unten. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11. Raise-Clip 17 ✓ 8.16.)
...
- **ESP-C (RENDER) — BLOCKER: braucht einen SAVESTATE eines aktiven Effekts (statischer Pfad erschöpft/verworren).**
  ...
  - **→ ESP-C PORT = ein dediziertes MEHR-SCHRITT-PHASE-PROJEKT** (wie die SCD-VM, aber für Effekte):
    ...
    - **🔄 (C2) IN ARBEIT — die ~52 Opcode-Handler `@0x800744a8` einzeln RE'n+porten** (volle Tabelle in `ghidra1_V2.txt` @0x800744a8,
      ...
      **Batch 2 = 0x09–0x0c** (per-Level-Loop-Counter: `inst[level+8]`-Index + u16-Array `@inst+level*8+0xa0`; 0x09 set-u16/0x0a
      dec-and-branch(pc+=3 wenn ≠0)/0x0b inc-index/0x0c `DAT_800aca3c(0x1a-Bit)`+`DAT_800b8520&0x80`-Gate). **Batch 3 = Draw/GTE/Field
