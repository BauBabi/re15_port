...
- **➡️ NÄCHSTER SCHRITT = ESP-C2 BATCH 3 = Ops 0x0d+.** ⚠️ **KORREKTUR (disasm geprüft): 0x0d ist KEIN Draw-Op**, sondern eine reichere Loop-Setup-Variante zu 0x09 (liest 6-Byte-Operand @pc+2, `cidx++`, schreibt per-Level-Arrays `@+0x60/+0x80/+0xa0`, speichert pc+Target `t1=pc+op`, `return CONT`). D.h. nach dem strukturellen Kern kommen noch **weitere Iterations-/Field-Setup-Ops**, die eigentlichen **GTE/Sprite-Draw-Builder liegen weiter hinten** in der Tabelle (einer ruft `FUN_80046a1c` = 40×30-SPRT). Reihenfolge also: 0x0d.. erst die restlichen Setup-Ops, dann die Draws. Hook = `re15_espvm_set_opcode(idx, fn)` / `re15_espvm_install_ops()`. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. **Mehr-Schritt-Projekt, NICHT in einem Rutsch.** (Die Draw-Ops sind ohne C3/Savestate evtl. nicht voll validierbar — Mechanismus belegen, nicht raten.) Volle Karte: ESP-C unten. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11. Raise-Clip 17 ✓ 8.16.)
