...
- **📋 VOLLE OPCODE-KARTE: `RE15_ESP_VM_OPCODES.md`** (neu, dieser Commit) — alle 95 Ops (0x00–0x5e) klassifiziert (Adresse/Status/Kategorie/Calls/Summary) + die Defer-Gruppen. Der 77-Agenten-Workflow `esp-vm-opcode-classify` hat 0x0d–0x5e disassembliert; jeder PORTED-Eintrag wurde vor dem Portieren selbst-disasm-verifiziert.
- **➡️ NÄCHSTER SCHRITT = die EXTERNEN Opcode-Gruppen (brauchen Integration/C3), nach Abhängigkeit geordnet:** (1) **VM-Register** `DAT_800b0fd0` (256× u16) → schaltet 0x1b/0x23/0x24/0x25/0x26/0x27 frei (rein RAM, gut portierbar als nächstes!); (2) **Work-Struct @+0x154** (0x32/0x3f/0x42/0x4a/0x3d/0x3e); (3) **Effekt-Pool `DAT_800b2368`** (0x4c/0x4d/0x2d/0x48 → ESP-B-Integration); (4) **Draw/GTE** (0x36/0x3a/0x40/0x41/0x47/0x55 + `FUN_80046a1c`-SPRT → Render-Pfad); (5) **Game-Flags/Cuts** (0x21/0x22/0x29/0x2a/0x3c/0x58/0x59/0x5e → `DAT_800aca3c`); (6) Audio 0x57, RNG 0x28, Entity-Spawn 0x44. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11.)
