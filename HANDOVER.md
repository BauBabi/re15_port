...
- **✅ ESP-C2 BATCH 6 (dieser Commit): GOSUB `0x18`** byte-true (disasm-verifiziert @0x8003fbe8) — saved-pc→`+0x144[level]`, level++, neuer Level reinit (depth/cidx=0xff, sp), jump zu `offset_table[id@pc+1]`. Der Partner zu op19/op1-RETURN. **Der strukturelle VM-Control-Flow ist damit KOMPLETT** (2 Loop-Systeme + Counter + Register + Compare + GOSUB/RETURN). **32/95 portiert.** Test-Teil (9i) = GOSUB→END→Return-Roundtrip. (Der Agent hatte 0x18 fälschlich deferred — `DAT_800b3f70` ist nur die Bytecode-Basis = `code_base`.)
