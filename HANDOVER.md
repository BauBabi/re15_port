...
- **➡️ NÄCHSTER SCHRITT (offen):** (a) **Cross-Check ausweiten** auf die scd_vm.c-Handler, die ich NICHT byte-true RE't habe (0x06/IF, 0x14/CASE, 0x17/GOTO, 0x21/CK, 0x22/SET, 0x2A, 0x2D, 0x37–0x3C, 0x47, 0x4A/0x4B, 0x50–0x5E) — gegen `RE15_SCD_OPCODES_REFERENCE.md` + frischen Disasm (weitere Bugs wie 0x35 finden); (b) **C3-Render** an scd_vm.c andocken (Draw-Ops fehlen, nur length-stubs; Bytecode-Quelle = main/sub SCD via RDT+0x40/+0x44, schon geparst); (c) anderes Thema (Combat/Raum/Game-Over).
