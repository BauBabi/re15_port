...
- **➡️ NÄCHSTER SCHRITT = ESP-C2 BATCH 3 = die Draw-/GTE-/Field-Ops 0x0d+** (0x0d/0x0f sind die großen GTE/Sprite-Builder; ein Draw-Op = der 40×30-SPRT `FUN_80046a1c` → PC-Sprite-Quad). Hook = `re15_espvm_set_opcode(idx, fn)` / `re15_espvm_install_ops()`. **C3** = Bytecode-Quelle (`DAT_800b3f70 = *(DAT_800ac778+0x40/0x44)`) + TIM/UV-LUTs. **Mehr-Schritt-Projekt, NICHT in einem Rutsch.** (Die Draw-Ops sind ohne C3/Savestate evtl. nicht voll validierbar.) Volle Karte: ESP-C unten. (Andere Optionen: Fade/Game-Over-UI · neuer Raum/Gegner · C11. Raise-Clip 17 ✓ 8.16.)
