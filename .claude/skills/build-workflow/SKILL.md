---
name: build-workflow
description: Generiert die CMake-Build-Befehle für die RE1.5-Port-Targets PC (SDL2/OpenGL) und PSX (PSn00bSDK) und diagnostiziert Build-Fehler (Compiler/Linker/CMake) mit Datei, Zeile und Lösungsvorschlag. Verwenden beim Bauen von re15_port, beim Ausführen der Tests (ctest) oder bei Build-Fehlern.
---
...
> Hinweis (Windows/PowerShell): `rm -rf build` entspricht `Remove-Item -Recurse -Force build`, und `&&` wird in Windows PowerShell 5.1 nicht unterstützt — Befehle einzeln bzw. mit `;` ausführen.
