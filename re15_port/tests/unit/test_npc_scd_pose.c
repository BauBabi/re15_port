...
#define CLIP3_LEN 26   /* s_irons_clip_len[3] (EM040) */
...
static void seed_pose(re15_actor_t *a, uint8_t type)
{
    ...
    a->motion = 3; a->anim_frame = 0; a->anim_flags = 0;
}
...
static int run_pose(re15_actor_t *a, const char *tag)
{
    ...
    if (a->motion != 3)                 { fprintf(stderr, "FAIL(%s): pose clip stomped — motion %d != 3\n", tag, a->motion); fail = 1; }
    ...
    if (a->anim_frame != CLIP3_LEN - 1) { fprintf(stderr, "FAIL(%s): not held at last frame — %d != %d\n", tag, a->anim_frame, CLIP3_LEN-1); fail = 1; }
    ...
    printf("  (%s) motion=%d state=%d anim_frame=%d (held at %d) wrapped=%d hp=%d\n",
           tag, a->motion, a->state, a->anim_frame, CLIP3_LEN-1, wrapped, a->hp);
    return fail;
}
