...
static int32_t s_nach_scd = 0;
int32_t g_probe_vor=0, g_probe_nach=0;
static void frame(void)
{
    const unsigned char *raw; int len, id;
    { extern int32_t g_probe_vor, g_probe_nach; (void)g_probe_vor; }
    scd_vm_tick();
    s_nach_scd = g_actors[2].x;
    ...
    if (s_nach_scd != g_actors[2].x) {
        static int n=0; if (n<6) { n++;
            printf("  [ORDNUNG] nach SCD x=%d  ->  nach game_step x=%d\n",
                   (int)s_nach_scd, (int)g_actors[2].x); }
    }
}
...
static int s_pin = 0;   /* Spieler jedes Bild auf die Startposition klemmen */
static int s_frames = 1600;
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
        ...
        else if (strcmp(argv[i], "pin") == 0) s_pin = 1;
        else if (strncmp(argv[i], "n=", 2) == 0) s_frames = atoi(argv[i]+2);
    ...
    printf("\n-- C) 1600 Bilder Kampf (Spieler steht) — wo geht Birkin hin? --\n");
    int32_t bx_min = 0x7fffffff, bx_max = -0x7fffffff;
    int bslot = -1;
    for (int s = 1; s < RE15_ACTOR_MAX; s++)
        if (g_actors[s].active && g_actors[s].type == 0x36) bslot = s;
    printf("  Boss-Slot = %d\n", bslot);
    int32_t pin_x = pl->x, pin_z = pl->z;
    int erst3000 = -1, erst1500 = -1, erst800 = -1;
    for (int f = 0; f < s_frames; f++) {
        if (s_pin) { pl->x = pin_x; pl->z = pin_z; }
        frame();
        if (s_pin) { pl->x = pin_x; pl->z = pin_z; }
        if (bslot >= 0) {
            long ad = (long)pl->x - (long)g_actors[bslot].x; if (ad < 0) ad = -ad;
            if (erst3000 < 0 && ad <= 3000) erst3000 = f;
            if (erst1500 < 0 && ad <= 1500) erst1500 = f;
            if (erst800  < 0 && ad <=  800) erst800  = f;
        }
        if (s_posset && bslot >= 0 && !s_posset_done &&
            g_actors[bslot].grid_id == 0x13 && g_actors[bslot].x == -14700) {
            g_actors[bslot].x = 1200; g_actors[bslot].z = -23350;
            s_posset_done = 1;
            printf("  [Vergleich] Pos_set(1200,-23350) eingesetzt bei Bild %d\n", f);
        }
        if (bslot >= 0) {
            if (g_actors[bslot].x < bx_min) bx_min = g_actors[bslot].x;
            if (g_actors[bslot].x > bx_max) bx_max = g_actors[bslot].x;
        }
        if ((f % 200) == 0 || f == s_frames - 1) dump(1000 + f);
    }
    printf("\n  ERSTMALS |dx|<=3000 bei Bild %d, <=1500 bei %d, <=800 bei %d  (-1 = nie, %d Bilder gefahren)\n",
           erst3000, erst1500, erst800, s_frames);
    if (bslot >= 0) {
        printf("\n  Birkin-X ueber 1600 Bilder: %d .. %d   (Spieler x=%d)\n",
               (int)bx_min, (int)bx_max, (int)pl->x);
        printf("  Kleinste erreichte Distanz in x: %ld\n",
               (long)((long)pl->x - (long)bx_max));
        printf("  Kamera-12-Sichtfeld laut RVD: x 2703..12249 — Birkin je darin? %s\n",
               (bx_max >= 2703 && bx_min <= 12249) ? "JA" : "NEIN");
    }
    return 0;
}
