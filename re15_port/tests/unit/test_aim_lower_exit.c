 ...
 * AIM_W sentinel. The fix drops to the idle sentinel (200) the same tick the LOWER exits.
 * (2026-08-08: AIM_W wurde von 213 auf 215 getrennt — die 213-Kollision mit IDLE_HURT1 ist
 * strukturell weg; die Exit-Invariante bleibt, weil ein stehengelassener AIM_W-Sentinel ohne
 * aktiven Render-Override fuer anim_select unaufloesbar ist.)
 */
...
/* player_common.c — der AIM_W-Sentinel (muss mit RE15_MOTION_AIM_W uebereinstimmen). */
#define AIM_W_SENTINEL 215
...
int main(void)
{
    ...
    pl->hp = 100;                 /* FULL HP -> AIM_W motion after aim ends can ONLY be the bug */
    pl->state = 0;
    ...
    int fail = 0;
    printf("=== aim LOWER exit must not leave the AIM_W sentinel (215) ===\n");
     ...
     * after, motion must have LEFT the AIM_W sentinel (sonst ist der Sentinel fuer anim_select
     * unaufloesbar). Track the exact tick aim goes inactive and assert motion != 215 from then on. */
    ...
    if (!inactive_seen) {
        ...
        fprintf(stderr, "FAIL: motion left at AIM_W(215) while aim INACTIVE (tick %d) "
                        "-> unaufloesbarer Sentinel = the 'land' bug class\n", bad_frame); fail = 1;
    } else if (exit_motion == AIM_W_SENTINEL) {
        fprintf(stderr, "FAIL: exit-tick motion == AIM_W (the leak)\n"); fail = 1;
    } else {
        printf("  aim released cleanly: exit-tick motion = %d (idle/locomotion, not AIM_W)\n",
               exit_motion);
    }
