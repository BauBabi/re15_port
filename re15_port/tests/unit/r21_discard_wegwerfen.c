...
static void teil_i(void)
{
    ...
    printf("  Das SCHLIESS-Bild: ohne den frozen()-Ausweg waeren %ld Spannen-Bilder ohne\n"
           ...
           "  aus dem Ueberhang des Runde-22-Modells (gemessen 887 Bilder ueber alle\n"
           "  gefahrenen Stellen, groesstes Einzelfenster 182) ist 1 Bild je\n"
...
static void teil_j(void)
{
    ...
    /* ⛔ BERICHTIGT: rueck_summe ist die SUMME ueber die Stellen, nicht der Weg je
     *    Stelle. Je Stelle sind es gemessen 75..101 Einheiten. Die alte Zeile nannte die
     *    Summe "je Stelle" und war damit um den Faktor 7..9 zu gross. */
    printf("  RUECKBAU des SCHLIESS-Bildes: %ld Einheiten SUMME ueber %d von %d Stellen\n"
           "  (je Stelle 75..101) — so viel Weg traegt die eine Zeile re15_discard_frozen()\n"
           "  in EINEM Bild.\n"
 ...
 * ========================================================================= */
/* Ein Lauf von TEIL M. `mit_besitz` ist der Unterschied, auf den es ANKOMMT: mit Besitz
 * geht die Abfrage auf (und beendet den Nachhall), ohne Besitz nicht — dann laeuft der
 * Nachhall ungestoert und ist ZAEHLBAR.
 * ⛔ BERICHTIGT: nicht der EINZIGE Unterschied. Die beiden Laeufe setzen auch
 * verschiedene Messfenster (s. die Schranke weiter unten, "121" = Schranke+1). Wer den
 * Vergleich als reines Ein-Variablen-Experiment zitiert, zitiert zu stark. */
