...
int main(int argc, char **argv)
{
    ...
    /* ⛔ DIE FALLEN-SCHWELLE, im Riegel SELBST ausgerechnet (berichtigt 2026-09-22).
     *
     * Die erste Fassung verglich den groessten Ein-Bild-Weg mit der DUENNSTEN WAND
     * (gemessen 2936 = density + 2*(450+0x12)). Das ist die falsche Bezugsgroesse und
     * der Riegel meldete deshalb am KAPUTTEN Stand bei einem Weg von 2112 woertlich
     * "ok" — gruen am Defekt, also wertlos (Memory reai-v2-schwaches-mass).
     *
     * Massgeblich ist nicht, ob ein Sprung eine Wand DURCHMISST, sondern ob er auf
     * einem unbegehbaren Punkt LANDET: die Klemme loest den Fall nur, solange der
     * Zielpunkt noch in ihrem Einzugsbereich liegt. Diese Schwelle haengt an der
     * Zellgeometrie des Raums, nicht an einer einzelnen Wand — deshalb wird sie hier
     * gemessen: ueber alle begehbaren Startpunkte des Kampfstreifens und 16 Richtungen
     * der kleinste Weg, der nach Klemme unbegehbar endet.
     *
     * Wahrheit fuer "begehbar" ist dieselbe wie ueberall in dieser Sonde: der
     * Klemmpfad (begehbar(), s.o.), nicht der Containment-Scan. */
    int32_t falle_schwelle = 0x7fffffff;
    long falle_proben = 0, falle_faelle = 0;
    {
        static const int SIN12[16] = {    0,  1567,  2896,  3784,  4096,  3784,  2896,  1567,
                                          0, -1567, -2896, -3784, -4096, -3784, -2896, -1567 };
        int32_t sx, sz, w, d;
        for (sx = 5200; sx <= 13800; sx += 400) {
            for (sz = -26400; sz <= -16400; sz += 400) {
                if (!begehbar(sx, sz, 0)) continue;
                for (w = 0; w < 16; w++) {
                    int32_t dx1 = SIN12[(w + 4) & 15], dz1 = SIN12[w];   /* cos, sin in Q12 */
                    for (d = 200; d <= 4000; d += 50) {
                        ...
                        if (!begehbar(nx, nz, 0)) {
                            falle_faelle++;
                            if (d < falle_schwelle) falle_schwelle = d;
                            break;                       /* groessere Wege sind nicht kleiner */
                        }
                    }
                }
            }
    ...
    printf("  FALLEN-SCHWELLE gemessen: kleinster Ein-Bild-Weg, der nach der Klemme"
           " unbegehbar endet = %d  (%ld Proben, %ld Faelle)\n",
           (int)falle_schwelle, falle_proben, falle_faelle);
    ...
    if (!strcmp(mode, "pin")) {
        ...
        PIN(falle_schwelle < 0x7fffffff && falle_proben > 100000,
            "die Fallen-Schwelle ist ueberhaupt messbar: %d aus %ld Proben",
            (int)falle_schwelle, falle_proben);
        PIN(m_maxsprung < falle_schwelle,
            "groesster Ein-Bild-Weg des Spielers %d bleibt unter der GEMESSENEN"
            " Fallen-Schwelle %d — ein Weg darueber landet auf einem Punkt, den die"
            " Klemme nicht mehr loest", (int)m_maxsprung, (int)falle_schwelle);
