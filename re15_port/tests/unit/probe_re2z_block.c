/**
 * @file probe_re2z_block.c  (MESSSONDE, Kopie von test_re2_zombie_abc.c)
    ...
    for (int seed = 0; seed < seeds; seed++) {
        ...
        for (int f = 0; f < budget; f++) {
            ...
            for (int s = 1; s < RE15_ACTOR_MAX; s++) {
                ...
                if (seed == g_trace_seed) {
                    static uint64_t lastsig[RE15_ACTOR_MAX];
                    static int      sig_init = 0;
                    if (!sig_init) { for (int q = 0; q < RE15_ACTOR_MAX; q++) lastsig[q] = ~0ull; sig_init = 1; }
                    uint64_t sig = ((uint64_t)e->state << 48) | ((uint64_t)e->sub_state_1 << 40) |
                                   ...
                                   ((uint64_t)(e->anim_frame & 0xff) << 56);
                    if (sig != lastsig[s]) {
                        printf("  T seed=%d slot=%d f%-4d st=%u s1=%2u s2=%2u 1D3=%02X 10E=%04X "
                               "21A=%04X grid=%02X clip=%2d af=%3u len=%2d hp=%4d 93=%02X t15A=%d\n",
                               seed, s, f, e->state, e->sub_state_1, e->sub_state_2,
                               e->re2z_self1d3, e->re2z_f10e, e->re2z_flags21a, e->grid_id,
                               (int)e->motion, (unsigned)e->anim_frame, re15_actor_clip_len(e),
                               e->hp, e->hit_react, (int)e->re2z_t15a);
                        lastsig[s] = sig;
                    }
                }
                ...
                if (blocked) {
                    ...
                    if (f - block_start[s] == cwin) {
                        ...
                        printf("     [C-VORFALL] seed=%d slot=%d f=%d (ab f=%d) st=%u s1=%u s2=%u hp=%d "
                               "1D3=0x%02x 10E=0x%04x clip=%d af=%u len=%d 93=0x%02x t15A=%d\n",
                               seed, s, f, block_start[s], e->state,
                               e->sub_state_1, e->sub_state_2, (int)e->hp, e->re2z_self1d3,
                               e->re2z_f10e, (int)e->motion, (unsigned)e->anim_frame,
                               re15_actor_clip_len(e), e->hit_react, (int)e->re2z_t15a);
                        { extern int re15_actor_clip_len_legacy(const re15_actor_t *a);
                          extern int re15_actor_uses_loco_bank(const re15_actor_t *a);
                          printf("     [C-DETAIL] typ=0x%02X act=%d grid=0x%02X frac=%u flags=0x%04X "
                                 "initdly=%u freeze=%u blend=%u len=%d legacy=%d loco?=%d x=%d z=%d\n",
                                 e->type, e->active, e->grid_id, (unsigned)e->anim_frac,
                                 (unsigned)e->anim_flags, (unsigned)e->motion_init_delay,
                                 (unsigned)e->anim_freeze, (unsigned)e->anim_blend_rate,
                                 re15_actor_clip_len(e), re15_actor_clip_len_legacy(e),
                                 re15_actor_uses_loco_bank(e),
                                 (int)e->x, (int)e->z);
                          re15_actor_t *P = &g_actors[RE15_ACTOR_SLOT_PLAYER];
                          printf("     [C-SPIELER] st=%u s1=%u s2=%u hp=%d act=%d mot=%d af=%u x=%d z=%d | room=0x%04X\n",
                                 P->state, P->sub_state_1, P->sub_state_2, (int)P->hp, P->active,
                                 (int)P->motion, (unsigned)P->anim_frame, (int)P->x, (int)P->z,
...
int main(int argc, char **argv)
{
    ...
    const int SEEDS  = (argc > 1) ? atoi(argv[1]) : 64;
    const int FRAMES = (argc > 2) ? atoi(argv[2]) : 900;
    const int CWIN   = (argc > 3) ? atoi(argv[3]) : 200;
    const int AMIN   = (argc > 4) ? atoi(argv[4]) : 30;
    { const char *ts = getenv("RE15_TRACE_SEED"); if (ts) g_trace_seed = atoi(ts); }
    printf("SONDE: SEEDS=%d FRAMES=%d CWIN=%d AMIN=%d trace_seed=%d\n",
           SEEDS, FRAMES, CWIN, AMIN, g_trace_seed);
