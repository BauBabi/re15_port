...
/* Die 6x8-Ersatzschrift ist ENTFERNT (Nutzer 2026-08-03: "diesen fallback font will ich
        ...
        if (W > 0 && H > 0) {
            ...
            if (rgba) {
                ...
                for (int i = 0; i < W * H; i++) {
                    ...
                    /* PSX-TEXTURREGEL (psx-spx, GPU-Texturen): der FARBWERT 0x0000 ist
                     * VOLL TRANSPARENT — nicht Schwarz. Der Port malte jeden Index != 0
                     * opak und legte damit einen schwarzen Kasten hinter die Namens-/
                     * PROFILE-Texte der Charakterauswahl (Nutzer-Report 2026-08-03: "im
                     * Original sind sie transparent"). Nur echtes Schwarz mit gesetztem
                     * STP-Bit (0x8000) waere opak schwarz. */
                    if (c == 0x0000) { rgba[i] = 0; continue; }
...
{
    /* ERSATZSCHRIFT ENTFERNT (Nutzer 2026-08-03: "diesen fallback font will ich gar nicht mehr
     * haben"). Alles Sichtbare laeuft ueber die ECHTE TEX.TIM-Spielschrift
     * (re15_render_pc_game_text / _game_codes / _msg_text / _item_prompt). Diese beiden
     * Wrapper bleiben nur als API-Stummel, damit fremde Aufrufer nicht brechen — sie
     * zeichnen bewusst NICHTS. */
    (void)x; (void)y; (void)text; (void)n;
    return 0;
}
...
/* ERSATZSCHRIFT ENTFERNT (Nutzer 2026-08-03). Der einzige verbliebene Aufrufer ist der
 * Custom-Debug-Opcode 0xFE der SCD-VM (scd_vm.c:1132) — der zeichnet jetzt in die echte
 * Spielschrift; alles Spielsichtbare lief ohnehin schon ueber TEX.TIM. */
void re15_debug_text(int x, int y, int z, const char *text)
{
    (void) z;
    if (!text) return;
    re15_render_pc_game_text(x, y, text, 0);
}
