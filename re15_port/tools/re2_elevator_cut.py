...
OUT_INC = os.path.join(REPO, "re15_port", "engine", "src", "gen", "re2_elev_bank.inc")
...
def main():
    ...
    with open(OUT_INC, "w", newline="\n") as o:
        o.write("/* AUTO-GENERATED by re15_port/tools/re2_elevator_cut.py -- DO NOT EDIT.\n"
                " * Satz-TOC der Mini-Bank shared_assets/RE2/ELEVSE.VBS, genau wie der\n"
                " * ENEMSE-Bank-TOC (re2_ems_toc.inc): der Satz steht in der Datei, seine\n"
                " * Groessen im Code. Satzformat audio_pc.c:1000-1007 / FUN_8005a09c:\n"
                " * [SE-Map @0 .. vh_off)[VH pBAV @vh_off][Trailer, u32 vh_off @size-8][VBD]. */\n")
        o.write("#define RE2_ELEV_EDT_OFF   %du\n" % 0)
        o.write("#define RE2_ELEV_EDT_SIZE  %du\n" % len(edt_rec))
        o.write("#define RE2_ELEV_VBD_OFF   %du\n" % len(edt_rec))
        o.write("#define RE2_ELEV_VBD_SIZE  %du\n" % len(vbd))
        o.write("/* Erwartete Wellen-Groessen (ROOM21B0.RDT @0x1BA34 / @0x1FA44). */\n")
        o.write("#define RE2_ELEV_VAG1_SIZE %du\n" % VAG_LEN[0x11])
        o.write("#define RE2_ELEV_VAG2_SIZE %du\n" % VAG_LEN[0x12])
        o.write("/* Die zwei SE-Ids, die RE2 vor die beiden bit28-Pulse setzt. */\n")
        o.write("#define RE2_ELEV_SE_RIDE    0x11\n")
        o.write("#define RE2_ELEV_SE_ARRIVE  0x12\n")
