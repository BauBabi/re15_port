...
TABELLE 3 -- ENEMSE-BANK-WAHL: die Raum-PAAR-Tabelle @0x800a7400 (Datei 0x97C00), 73+1 Zeilen
x {u8 kindA, u8 kindB}, terminiert von kindA == 0xFF. Sie ist der SCHLUESSEL, mit dem
FUN_80052b38 (RE2_Quellcode_V2/FUN_80052b38.c, Aufruf @0x80053610 im Raum-Setup) die
ENEMSE-Bank-NUMMER DAT_800d424b bestimmt -- exakt der Index, den der Lader FUN_8005a09c
danach als `bank*0x10` in TABELLE 2 einsetzt (@FUN_8005a09c:17-29).
Regel (byte-true aus dem Decompilat + Disasm @0x80052b40..0x80052c2c):
  fuer jede Zeile r (Index = Bank):
    (a) r.kindA == RAUM_A && (RAUM_B == 0 || RAUM_B == r.kindB)  -> Treffer, zweite Haelfte = r.kindB
    (b) r.kindB == RAUM_A && (RAUM_B == 0 || RAUM_B == r.kindA)  -> Treffer, zweite Haelfte = r.kindA
  kein Treffer -> 0xFF (= Lader bricht ab, KEINE Bank).
  Danach setzt FUN_80052b38 bei jeder Entity, deren kind (+0x1FA) die ZWEITE Haelfte ist,
  word0 |= 0x2000 -- genau das Bit, das der SE-Trigger FUN_8005bd6c als "+0x10-Map-Haelfte"
  liest. RAUM_A/RAUM_B selbst fuellt der Enemy-Spawn @0x8005728c-0x800572b8:
    kind == A -> nichts;  A == 0 -> A = kind;  sonst -> B = kind
  (zweiter, identischer Spawn-Pfad @0x80057920-0x8005794c).
"""
import os
import struct
...
PAIR_TBL_VA = 0x800A7400     # TABELLE 3 (s.u.)
...
def main():
    ...
    # --- Paar-Tabelle (Bank-Wahl) ---
    pair_off = file_off(PAIR_TBL_VA)                        # 0x97C00
    pairs = list(d[pair_off:pair_off + (ENEMSE_BANKS + 1) * 2])
    # Anker: Zeile 11 = {0x10,0x00} (reiner Zombie-Cop), 21 = {0x21,0x00} (Kraehe),
    # 31 = {0x00,0x20} (Hund als ZWEITE Haelfte); Zeile 73 terminiert mit 0xFF.
    assert pairs[11*2:11*2+2] == [0x10, 0x00], pairs[11*2:11*2+2]
    assert pairs[21*2:21*2+2] == [0x21, 0x00], pairs[21*2:21*2+2]
    assert pairs[31*2:31*2+2] == [0x00, 0x20], pairs[31*2:31*2+2]
    assert pairs[73*2] == 0xFF, hex(pairs[73*2])            # Schleifen-Terminator @0x80052bec
    ...
    with open(OUT, "w", newline="\n") as f:
        ...
        lines = ["/* Raum-PAAR-Tabelle @0x800a7400 (Datei 0x97C00): (73+1) x {u8 kindA, u8 kindB},",
                 " * Zeilenindex == ENEMSE-Bank (FUN_80052b38 -> DAT_800d424b -> FUN_8005a09c),",
                 " * terminiert von kindA == 0xFF. */",
                 f"static const uint8_t s_re2_enemse_pairs[{len(pairs)}] = {{"]
        for i in range(0, len(pairs), 12):
            lines.append("    " + ", ".join(f"0x{b:02x}" for b in pairs[i:i+12]) + ",")
        lines.append("};")
        f.write("\n".join(lines) + "\n")
    print(f"wrote {OUT}: cdemd0 {CDEMD0_TOC_N}x2 u32, enemse {ENEMSE_BANKS}x4 u32, "
          f"pairs {len(pairs)//2} rows")
