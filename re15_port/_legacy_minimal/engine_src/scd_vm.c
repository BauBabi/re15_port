...
/* RE1.5-Opcode-Bytelängen (0x00..0x8F). Erlaubt das sichere Überspringen noch
 * nicht implementierter Opcodes, ohne den Bytecode-Stream zu desynchronisieren.
 * 0 (für 0x1F, 0x90..0xFF) wird im Handler als 1 behandelt. */
static const uint8_t scd_op_length[256] = {
  /*0x00*/ 1,2,1,4,4,2,4,4,2,4,3,1,1,6,2,4,
  /*0x10*/ 2,4,2,4,6,4,2,6,2,2,2,6,1,1,1,1,
  /*0x20*/ 1,4,4,6,4,3,6,4,1,2,1,4,20,34,3,4,
  /*0x30*/ 1,1,8,8,4,3,12,4,3,4,16,32,2,4,6,4,
  /*0x40*/ 8,10,1,4,20,3,10,2,16,8,2,3,5,22,22,4,
  /*0x50*/ 22,4,4,3,6,6,6,4,4,4,6,4,4,4,4,2,
  /*0x60*/ 14,4,2,1,16,2,1,28,40,30,6,4,1,4,6,2,
  /*0x70*/ 1,1,16,8,4,22,3,4,6,1,16,16,6,6,6,6,
  /*0x80*/ 2,3,3,1,2,6,1,1,3,1,6,6,1,24,2,1
};

static int scd_op_unknown(scd_vm_t* vm, scd_thread_t* thread)
{
    uint8_t opcode = thread->pc[0];
    int len = scd_op_length[opcode];
    if (len <= 0) len = 1;

    (void)vm;

    /* Noch nicht implementiert: Opcode überspringen statt den Thread zu killen,
     * damit das Skript weiterläuft (Wirkung entfällt, Stream bleibt synchron). */
    RE15_WARN("SCD", "Opcode 0x%02X nicht implementiert — übersprungen (%d B)", opcode, len);
    thread->pc += len;
    return 1; /* continue */
}
