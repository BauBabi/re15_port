 ...
 *
 * Implementiert PSX-GTE-äquivalente Mathematik in Software für die PC-Plattform.
 * Die Sinustabelle enthält 1024 Einträge (Viertelwelle, 0° bis 90°) und nutzt
 * Symmetrie zur Abdeckung aller 4096 Winkel.
 *
 * Tabellenwerte: sin_table[i] = round(sin(i * 2π / 4096) * 4096)
 * für i = 0..1023 (erstes Viertel, 0° bis knapp unter 90°).
 *
 * Ergebnisse weichen maximal ±1 Pixel von der PSX-GTE-Referenz bei 320×240 ab.
 *
...
/* ============================================================================
 * Sinus-Lookup-Tabelle (Viertelwelle, 1024 Einträge, Q12)
 *
 * Generiert mit: round(sin(i * 2π / 4096) * 4096) für i = 0..1023
 * Werte reichen von 0 (sin 0°) bis 4096 (sin ~90°).
 *
 * Symmetrie-Nutzung:
 *   Q1 (0..1023):    sin(a)     = +sin_table[a]
 *   Q2 (1024..2047): sin(a)     = +sin_table[2048 - a]
 *   Q3 (2048..3071): sin(a)     = -sin_table[a - 2048]
 *   Q4 (3072..4095): sin(a)     = -sin_table[4096 - a]
 * ========================================================================= */
...
/**
 * Sinus im Q12-Format mit Viertelwellen-Symmetrie.
 *
 * 4096 Winkeleinheiten = 360°:
 *   Quadrant 1 (0..1023):    +sin_table[angle]
 *   Quadrant 2 (1024..2047): +sin_table[2048 - angle]
 *   Quadrant 3 (2048..3071): -sin_table[angle - 2048]
 *   Quadrant 4 (3072..4095): -sin_table[4096 - angle]
 */
int32_t re15_sin_q12(int16_t angle) {
    int32_t a = angle & RE15_ANGLE_MASK;  /* Wrap zu 0-4095 */
    int32_t index;
    int32_t value;

    if (a < 1024) {
        /* Quadrant 1: 0° bis 90° — direkte Tabellenlookup */
        value = (int32_t)sin_table[a];
    } else if (a < 2048) {
        /* Quadrant 2: 90° bis 180° — Spiegelung an 90° */
        index = 2048 - a;
        if (index >= 1024) index = 1023; /* Clamp für Grenzfall a==1024 */
        value = (int32_t)sin_table[index];
    } else if (a < 3072) {
        /* Quadrant 3: 180° bis 270° — negiert */
        index = a - 2048;
        value = -(int32_t)sin_table[index];
    } else {
        /* Quadrant 4: 270° bis 360° — negiert + gespiegelt */
        index = 4096 - a;
        if (index >= 1024) index = 1023; /* Clamp für Grenzfall a==3072 */
        value = -(int32_t)sin_table[index];
    }
 ...
 * cos(angle) = sin(angle + 1024) — Phasenverschiebung um 90°.
 */
int32_t re15_cos_q12(int16_t angle) {
    return re15_sin_q12(angle + RE15_ANGLE_QUARTER);
}
...
void re15_rot_matrix_identity(re15_rot_matrix_t* out) {
    memset(out, 0, sizeof(re15_rot_matrix_t));
    out->m[0][0] = Q12_ONE;  /* 4096 = 1.0 in Q12 */
    out->m[1][1] = Q12_ONE;
    out->m[2][2] = Q12_ONE;
}
...
void re15_rot_matrix_z(re15_rot_matrix_t* out, int16_t angle) {
    ...
    out->m[1][0] = (int16_t)s;
    out->m[1][1] = (int16_t)c;
    out->m[2][2] = (int16_t)Q12_ONE;
}

/**
 * Matrixmultiplikation: out = a * b
 * Alle Einträge in Q12. Multiplikation: (a_ij * b_jk) >> 12
 */
void re15_rot_matrix_mul(re15_rot_matrix_t* out,
                         const re15_rot_matrix_t* a,
                         const re15_rot_matrix_t* b) {
    int row, col, k;
    re15_rot_matrix_t result;

    for (row = 0; row < 3; row++) {
        for (col = 0; col < 3; col++) {
            int32_t sum = 0;
            for (k = 0; k < 3; k++) {
                sum += ((int32_t)a->m[row][k] * (int32_t)b->m[k][col]);
            }
            /* Q12 * Q12 → Q24, shift right by 12 to get Q12 result */
            result.m[row][col] = (int16_t)(sum >> Q12_SHIFT);
        }
    }
...
void re15_transform_vertex(const re15_rot_matrix_t* rot,
    ...
    /*
     * Berechnung: out = rot * v + trans
     *
     * Jede Matrix-Zeile dot Vertex-Spalte:
     *   out_x = (m[0][0]*vx + m[0][1]*vy + m[0][2]*vz) >> 12 + tx
     *   out_y = (m[1][0]*vx + m[1][1]*vy + m[1][2]*vz) >> 12 + ty
     *   out_z = (m[2][0]*vx + m[2][1]*vy + m[2][2]*vz) >> 12 + tz
     *
     * Nutzt int64_t für die Akkumulation um Overflow bei großen Vertices
     * zu vermeiden (Vertex-Bereich ±32767, Matrix ±4096 → Produkt max ±134M).
     */
    int64_t rx, ry, rz;

    rx = (int64_t)rot->m[0][0] * vx
       + (int64_t)rot->m[0][1] * vy
       + (int64_t)rot->m[0][2] * vz;

    ry = (int64_t)rot->m[1][0] * vx
       + (int64_t)rot->m[1][1] * vy
       + (int64_t)rot->m[1][2] * vz;

    rz = (int64_t)rot->m[2][0] * vx
       + (int64_t)rot->m[2][1] * vy
       + (int64_t)rot->m[2][2] * vz;

    *ox = (int32_t)(rx >> Q12_SHIFT) + trans->tx;
    *oy = (int32_t)(ry >> Q12_SHIFT) + trans->ty;
    *oz = (int32_t)(rz >> Q12_SHIFT) + trans->tz;
}

/* ============================================================================
 * Perspektiv-Projektion (GTE-Äquivalent)
 * ========================================================================= */
...
int re15_perspective_project(int32_t x3d, int32_t y3d, int32_t z3d,
    ...
    /*
     * PSX-GTE Perspektiv-Division:
     *   sx = (x * h) / z + cx
     *   sy = (y * h) / z + cy
     *
     * Sicherheitscheck: z muss > 0 sein (Vertex vor der Kamera).
     * Division durch z im Integer — passend zur GTE-Präzision.
     */
    if (z3d <= 0) {
        return -1;  /* Hinter der Kamera */
    }

    *sx_out = (int32_t)(((int64_t)x3d * h) / z3d) + cx;
    *sy_out = (int32_t)(((int64_t)y3d * h) / z3d) + cy;
