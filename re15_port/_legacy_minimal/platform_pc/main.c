...
#define RE15_START_ROOM     0x24   /* ROOM1240 — Intro-/Briefing-Raum (Startknoten) */
...
int main(int argc, char* argv[])
{
    ...
    if (rc != 0) {
        ...
        /* Initialen BSS-Hintergrund (Cut 0, in re15_room_load geladen) anzeigen */
    ...
    while (running) {
        ...
        if (rvd_data.count > 0) {
            ...
            if (new_cam != g_game.current_cut) {
                ...
                /* BSS-Chunk der neuen Kamera laden, dann anzeigen */
                if (re15_bss_load_chunk(g_game.current_stage, g_game.current_room,
                                        new_cam, g_game.bss_chunk) != RE15_IO_OK) {
                    memset(g_game.bss_chunk, 0, RE15_BSS_CHUNK_SIZE);
                }
                if (g_render->show_background) {
        ...
        if (g_scd.cam_change_pending) {
            ...
            if (re15_bss_load_chunk(g_game.current_stage, g_game.current_room,
                                    g_scd.cam_id, g_game.bss_chunk) != RE15_IO_OK) {
                memset(g_game.bss_chunk, 0, RE15_BSS_CHUNK_SIZE);
            }
            if (g_render->show_background) {
