...
| ~~U4~~ | ~~**Ifel_ck non-Ck condition eval** (Member_cmp real / Sce_key_ck+Cmp conservative-FALSE)~~ ✅ RESOLVED 2026-07-12 | scd_vm.c op_ck/op_cmp/op_member_cmp/op_sce_key_ck | — | The pre-eval look-ahead was replaced by opcode-native predicates (op_if enters the body; the first opcode IS the predicate and returns the dispatch boolean). **Cmp (0x23)** evaluates work_vars for real (all 7 operators, LAB_8003ff68); **Member_cmp (0x3E)** is real; **Ck (0x21)** verified in intro. The last conservative-FALSE gap was **Sce_key_ck (0x51)** — now wired byte-true to the held pad: `cond = (mask & g_scd_pad_held) ? param : param^1` (LAB_80042920; g_scd_pad_held = pad_current in PSX bit convention, same word the dialog FSM reads as & 0x4000 = CROSS, wf_6aad95ad). No-input default stays param^1 (regression-free) — only a held mask button changes a branch (e.g. ROOM1080's 8 per-frame input-poll predicates). ctest `unit_scd_key_ck` (10 checks). |
...
| ~~U10~~ | ~~**pri_common.c square-mask `(size>>4)*8`**~~ ✅ VERIFIED BYTE-TRUE 2026-07-13 (audit wf_27ae1ea7) | pri_common.c:104 | — | Disasm-verified against FUN_800392d4 @0x80039428/40: the original is `(size>>12)<<3` on the u16 size field = high-nibble×8 = exactly the port's `(mp[7]>>4)*8`. The Java reference (`rectangular = size==0`, `width=height=size`) is the one that is WRONG vs the PSX; the port's deviation from Java is the correct byte-true fix, not a bug. Whole depth/occlusion model also verified byte-consistent (masks at ot[depth], actors at ot[otz>>4]). |
